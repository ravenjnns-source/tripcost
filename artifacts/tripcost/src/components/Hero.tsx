...
const SAMPLE_CARDS = [
  { country: "Paris, France", flag: "🇫🇷", daily: "$95", styleKey: "standard" as const, color: "from-blue-500/20 to-teal-500/20" },
  { country: "Bali, Indonesia", flag: "🇮🇩", daily: "$38", styleKey: "budget" as const, color: "from-green-500/20 to-emerald-500/20" },
  { country: "Tokyo, Japan", flag: "🇯🇵", daily: "$110", styleKey: "standard" as const, color: "from-purple-500/20 to-pink-500/20" },
];
...
export function Hero() {
  ...
  const styleLabels: Record<string, string> = {
    budget: t.calculator.budget,
    standard: t.calculator.standard,
    luxury: t.calculator.luxury,
  };
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 w-full">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <motion.div initial="hidden" animate="visible" variants={stagger} className="text-white">
            <motion.div
              variants={fadeUp}
              className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-4 py-2 text-sm font-medium text-white/90 mb-6"
            >
            ...
            <motion.div variants={fadeUp} className="flex gap-8 mt-12 pt-8 border-t border-white/10">
              {[
                { value: "195+", label: t.hero.statsCountries },
                { value: "3", label: t.hero.statsTravelStyles },
                { value: "10", label: t.hero.statsLanguages },
              ].map((stat) => (
                <div key={stat.label}>
            ...
            <div className="relative">
              <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-6 shadow-2xl">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <div className="text-white font-semibold text-sm">{t.hero.sampleTrip}</div>
                    <div className="text-white/50 text-xs">{t.hero.sampleTripSub}</div>
                  </div>
                </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-white/50 text-xs">{styleLabels[card.styleKey]}</div>
                        </div>
                      </div>
                      <div className="text-end">
                        <div className="text-[#00b5c8] font-black text-lg">{card.daily}</div>
                        <div className="text-white/50 text-xs">{t.hero.perDay}</div>
                      </div>
                    </motion.div>
                  ))}
                </div>
                ...
                <div className="mt-5 pt-5 border-t border-white/10 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-white/60 text-xs">
                    ...
                    {t.hero.liveEstimates}
                  </div>
                  <div className="text-white/40 text-xs">{t.hero.updatedRegularly}</div>
                </div>
              </div>
